...
import SummarizeContribution from '/lib/modules/karar/summarize_contribution.js';
import ValidateContribution from '/lib/modules/karar/validate_contribution.js';
import {versions} from '/lib/configs/karar/data_models';
      ...
      try {
        ...
        if (isLatest === 'true') {
          history.slice(1).forEach(v => {
            try {
              s3DeleteKeys({
                bucket: 'karar-contributions',
                keys: [`${v.id}/karar_contribution_${v.id}.txt`]
              }).then(() => {
                console.log("esUploadActivatedContributionToS3", `Deleted ${v.id}/karar_contribution_${v.id}.txt`);
                s3DeleteKeys({
                  bucket: 'karar-contributions',
                  keys: [`${v.id}/`]
                }).then(() => {
          ...
          try {
            s3UploadObject({
                bucket: 'karar-contributions',
                key: `${id}/karar_contribution_${id}.txt`,
                body: contributionText
            }).then(() => {
              console.log("esUploadActivatedContributionToS3", `Uploaded ${id}/karar_contribution_${id}.txt to karar-contributions`);
            });
          } catch (e) {
        ...
        try {
          s3UploadObject({
            bucket: 'karar-activated-contributions',
            key: `${id}/karar_contribution_${id}.txt`,
            body: contributionText
          }).then(() => {
            console.log("esUploadActivatedContributionToS3", `Uploaded ${id}/karar_contribution_${id}.txt to karar-activated-contributions`);
          });
        } catch (e) {
